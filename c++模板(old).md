...
#### 1.1.1 Template  Class   的声明和定义

Template Class声明： 
```C++
template <class T> class ClassA;   
or  template <typename T> class ClassA;
```
...
`template` 是C++关键字，意味着接下来将定义一个模板。和函数一样，模板也有一系列参数。这些参数都被囊括在template之后的`< >`中。在上文的例子中， `typename T`便是模板参数。

这里的“模板形参”`T`，也同函数形参一样取成任何你想要的名字；`typename`则类似于例子中函数参数类型`int`，它表示模板参数中的`T`将匹配一个类型。除了 `typename` 之外，整型也可以作为模板的参数。

 `T`是模板的精髓，可以通过指定模板实参，将T替换成你所需要的类型。



例如:

 用`ClassA<int>`来实例化模板类ClassA，那么`ClassA<int>`可以等同于以下的定义：
...
可以看出，通过模板参数替换类型，可以获得很多形式相同的新类型，有效减少了代码量。

这种用法，我们称之为“泛型”（Generic Programming），它最常见的应用，即是STL中的容器模板类。



#### 1.1.2 模板的使用

对于C++来说，类型最重要的作用之一就是用它去产生一个变量。

例如:  我们定义了一个动态数组（列表）的模板类`vector`，它对于任意的元素类型都具有push_back和clear的操作，我们便可以如下定义这个类：
...
+ 第一步，`vector<int>`将`int`绑定到模板类`vector`上，获得了一个“普通的类`vector<int>`”；

+ 第二步，通过“vector<int>”  定义了一个变量。与“普通的类”不同，模板类是不能直接用来定义变量的。例如：

  ```c++
  vector unknownVector; // 错误示例
  通过类型绑定(<类型>)将模板类变成“普通的类”的过程，称之为模板实例化（Template Instantiate）。
  ```
...
ClassB<int, float> name
```

注意: 需要提供足够并匹配的参数，保证模板能正确的实例化。



#### 1.1.3 模板类的成员函数定义

**在成员函数实现的时候，必须要提供模板参数。**

由于**C++11正式废弃“模板导出”这一特性**，因此在模板类的变量在调用成员函数的时候，需要看到完整的成员函数定义。**注意 现在的模板类中的成员函数，通常都是以内联的方式实现。**
例如：
...
{
public:
    void clear(){ // Function body }
private:
    T* elements;
};
```

也可以将`vector<T>::clear`的定义部分放在类型之外，只不过这个时候的语法就显得蹩脚许多：
...
/**
void vector::clear()    // err
{
    // Function body
}
**/

// 正确的写法  // 但是新标准都是直接写成内联函数
template <class T1, class T2>                    // 模板参数
void vector<T1, T2> /*看起来像偏特化*/ ::clear()  // 函数的实现放在这里
{
    // Function body
}
```
...
class Subperson : public Pserson<int>         // 必须指定类型
{
    ....
}
```
...
class Subperson : public Pserson<T>         // 依然用T 代替类型
{
    ....
}
```
...
![image-20220809214452120](C:\Users\16658\AppData\Roaming\Typora\typora-user-images\image-20220809214452120.png)

![image-20220809214506623](C:\Users\16658\AppData\Roaming\Typora\typora-user-images\image-20220809214506623.png)
...
```
编译器并不是把函数模板处理成能够处理任何类型的

函数模板通过具体类型产生不同的函数

编译器会对函数模板**进行两次编译**，在声明的地方对模板代码本身进行编译，在调用的地方对参数替换后的代码进行编译。
```
...
#### 1.2.2 Template Function的声明和定义

模板函数的语法与模板类基本相同，也是以关键字`template`和模板参数列表作为声明与定义的开始。

**模板参数列表中的类型------可以出现在参数、返回值以及函数体中**。例子如下:

```C++
template <typename T> void foo(T const& v);

template <typename T1, typename T2, .... > T1 foo();
...
归根结底，模板无外乎两点：
   ...
   1. 函**数或者类里面，有一些类型我们希望它能变化一下，我们用标识符来代替它，这就是“模板参数”；**
...
比较常见的场景包括：泛型（最基本的用法）、通过类型获得相应的信息（型别萃取）、编译期间的计算、类型间的推导和变换（从一个类型变换成另外一个类型，比如boost::function）。
...
例如，两个 `int` 求和，那么套用类的模板实例化方法，我们可以这么写：
...
在调用 `Add<int>(a, b)` 的时候， `a` 和 `b` 匹配的都是那个 `T`。编译器就知道那个 `T` 实际上是 `int` 

``` C++
int a = 5;
int b = 3;

int result = Add(a, b);   // 编译器会将  `Add` 变成 `Add<int>`
```

注意 :   但是编译器不能面对模棱两可的答案。如下:
...
第一个参数 `a` 告诉编译器，这个 `T` 是 `int`。但是第二个参数 `b` ，告诉编译器说，你这个 `T`，其实是 `char`。
两个参数各自指导 `T` 的类型，编译器就不知道怎么做了。

在Visual Studio 2012下，会有这样的提示：
...
A<int> v;
foo(v);	      // 它能准确地猜到 T 是 int.  // 编译器可以绕过了A这个外套，猜到了 `T` 匹配的是 `int`
```

---------------

+ **编译器是没办法去根据返回值推断类型的。 函数调用的时候，返回值被谁接受还不知道。  例子1**
+ **需要 指定的模板参数，一定写在 能推导出来的模板参数 的 前面  例子2**。
...
float a = GetValue(0);	// err
int b = GetValue(1);	// err
```

如下修改后：

``` C++
float a = GetValue<float>(0);   // ok
int b = GetValue<int>(1);       // ok
```

​			==例子2:==

写一个模板函数叫 `c_style_cast`，执行的是C风格的转换。然后出于方便起见，你希望它能和 `static_cast` 这样的内置转换有同样的写法。

``` C++
DstT dest = c_style_cast<DstT>(src);
```

根据调用形式，有 `DstT` 和 `SrcT` 两个模板参数。参数只有一个， `src`，所以函数的形参当然是这么写了： `(SrcT src)`。实现也很简单， `(DstT)v`。
...
int v = 0;
float i = c_style_cast<float>(v);    // err
```
...
编译器对模板参数的顺序是有限制的：**先写需要指定的模板参数，再把能推导出来的模板参数放在后面**。
...
模板参数除了类型外（包括基本类型、结构、类类型等），也可以是一个整型数（Integral Number）。这里的**整型数比较宽泛，包括布尔型，不同位数、有无符号的整型，甚至包括指针。**

``` C++
template <typename T> class TemplateWithType;
template <int      V> class TemplateWithValue;   // 整型模板参数
```

 `typename` 相当于是模板参数的“类型”，告诉你 `T` 是一个 `typename`。

**整型模板参数最基本的用途，也是定义一个常数。**
...
注意:   模板的匹配是在编译的时候完成的，所以实例化模板的时候所使用的模板参数，必须要在编译期就能确定。例如以下的例子编译器就会报错：
...
template <float a> class E {};   // ERROR:  只能是整数类型
...
当然，除了单纯的用作常数之外，整型参数还有一些其它的用途。这些“其它”用途最重要的一点是让类型也可以像整数一样运算。
